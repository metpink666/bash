...
# 配置
SERVICE_NAME="metpink-sing-box"
INSTALL_DIR="/opt/metpink/test/$SERVICE_NAME"
...
show_menu() {
    clear
    echo "╔══════════════════════════════════════════════╗"
    echo "║          $SERVICE_NAME 管理面板              ║"
    echo "╚══════════════════════════════════════════════╝"
    echo ""
    echo "        ┌─────────────────────────────┐"
    echo "        │   [1] 检测运行状态          │"
    echo "        │   [2] 安装 $SERVICE_NAME    │"
    echo "        │   [3] 配置SK5代理           │"
    echo "        │   [4] 启动服务              │"
    echo "        │   [5] 查看配置              │"
    echo "        │   [6] 停止服务              │"
...
check_status() {
    ...
    if pgrep -f "$SERVICE_NAME" > /dev/null; then
        ...
        echo "进程信息:"
        ps aux | grep -E "$SERVICE_NAME" | grep -v grep
    else
...
configure_sk5() {
    ...
    if [ ! -f "$INSTALL_DIR/$SERVICE_NAME" ]; then
        echo -e "${RED}请先安装 $SERVICE_NAME${NC}"
        echo "按回车键返回..."
        read
        return 1
    fi
...
start_service() {
    ...
    cd "$INSTALL_DIR"
    nohup "./$SERVICE_NAME" run -c "$CONFIG_FILE" > "$INSTALL_DIR/sing-box.log" 2>&1 &
    
    sleep 2
    
    if pgrep -f "$SERVICE_NAME" > /dev/null; then
        echo -e "${GREEN}✅ $SERVICE_NAME 启动成功！${NC}"
        echo "日志文件: $INSTALL_DIR/sing-box.log"
    else
        echo -e "${RED}❌ $SERVICE_NAME 启动失败${NC}"
        echo "请查看日志文件: $INSTALL_DIR/sing-box.log"
    fi
